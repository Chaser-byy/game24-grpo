...
- **Tree of Thoughts**：使用其官方困难数据切片作为评测，并提供模型驱动的状态树搜索；
  另保留程序化 exhaustive ToT 作为 solver-style oracle 上界。
...
game24/
  ...
  trajectory.py       # 经过验证的分步推理轨迹，用于 SFT
  splits.py           # ToT 留出与无泄漏划分
  inference.py        # Qwen/LoRA 生成和多候选采样
  evaluation.py       # accuracy@1、pass@k、难度和无解评测
  model_tot.py        # 模型提出分支、程序验证状态的 ToT
  tot.py              # 程序化 arithmetic ToT / oracle 上界
scripts/
  prepare_experiment_data.py # 正式 Game24 数据协议
  prepare_countdown_data.py  # Countdown OOD 扩展
  train_grpo.py              # 单 GPU LoRA-GRPO
  train_sft_warmup.py        # 轨迹式 SFT warmup
  evaluate_baseline.py       # 基线/adapter 统一评测
  evaluate_verified_decoding.py # Best-of-N + verifier selection
  evaluate_model_tot.py      # 模型驱动 ToT
  evaluate_tot.py            # 程序化 ToT oracle/beam search
  evaluate_strategy_suite.sh # Single/Best-of-N/Model-ToT 套件
...
太稀疏，同组采样里很少出现正样本。此时可先用项目内精确搜索生成训练题参考解和
分步推理轨迹，做一个 LoRA SFT warmup，再把 LoRA 合并成普通 CausalLM 作为 GRPO 初始策略。
SFT 轨迹会写明每一步当前状态、选择的两个表达式、执行的运算、剩余状态和最终检查。
...
模型驱动 Tree-of-Thought 是推荐的测试时搜索实验。程序维护结构化状态并用 Fraction
执行运算，模型只提出下一步候选操作，如 `0 * 1`。程序会拒绝非法索引、除零和错误状态，
再用启发式 beam 保留中间状态继续搜索：

```bash
python scripts/evaluate_model_tot.py \
  --model outputs/grpo_after_sft_4090 \
  --data data/processed/test_hard.jsonl \
  --output outputs/model_tot_hard.jsonl \
  --beam-size 5 \
  --candidates-per-state 4 \
  --branch-samples 2
```

同一模型的 Single、Best-of-N 和 Model-ToT 可以用套件一次比较：

```bash
bash scripts/evaluate_strategy_suite.sh \
  outputs/grpo_after_sft_4090 \
  data/processed/test_hard.jsonl \
  outputs/eval_suite_grpo_after_sft \
  32
```

程序化 Tree-of-Thought 可作为 oracle/solver-style 上界。这里不调用模型，每一步枚举两个
剩余表达式的 `+ - * /` 合并，并由同一个 verifier 检查终点。默认 exhaustive，因此不应和
模型 greedy 能力混为一谈。
